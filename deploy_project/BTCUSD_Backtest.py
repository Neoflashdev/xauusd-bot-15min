...
def run_quant_validation_engine(trades: list, df: pd.DataFrame, df_ml: pd.DataFrame):
    ...
    print(f"\n  PROP FIRM FAILURE PROBABILITY (10% Max DD Limit):")
    print(f"    At 1.00% Risk per trade  : {(np.sum(sim_dd <= -10.0) / 10000 * 100):.1f}%")
    print(f"    At 0.75% Risk per trade  : {(np.sum(sim_dd <= -13.33) / 10000 * 100):.1f}%")
    print(f"    At 0.50% Risk per trade  : {(np.sum(sim_dd <= -20.0) / 10000 * 100):.1f}%")
    
    print(f"\n  Expected Max Losing Streak : {np.median(sim_ls):.0f} trades")
    ...
    fail_prob_50 = np.sum(sim_dd <= -20.0) / 10000
    if fail_prob_50 > 0.25: fail_reasons.append(f"Monte Carlo probability of failure at 0.5% risk ({(fail_prob_50*100):.1f}%) > 25%")
...
CONFIG = {
    ...
    "path_m15": "BTCUSD_15min.csv",
    "path_4h":  "BTCUSD_4H.csv",
...
def load_csv(path: str, label: str) -> pd.DataFrame:
    ...
    for col in df.columns:
        ...
        if c_lower in ['date', 'time', 'datetime', 'date time', 'local time', 'open_time']:
            col_map[col] = 'Date'
...
def compute_sl_tp(direction: int,
    ...
    if direction == 1:  # LONG
        ...
        # Enforce maximum SL distance to prevent 'black swan' locking
        if sl_distance > 5.0 * atr:
            sl_distance = 5.0 * atr
            structure_sl = entry_price - sl_distance
...
def main():
    print("=" * 60)
    print("  BTCUSD M15 MSB BACKTESTING SYSTEM")
